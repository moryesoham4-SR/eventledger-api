...
def _safely(conn, fn):
    """Runs `fn()` inside its own SAVEPOINT. PostgreSQL aborts the WHOLE
    transaction the instant any statement fails and refuses further commands
    (including the final commit in get_db()) until a rollback happens — so
    notification side-effects (which touch a table we can't fully verify the
    schema of from here) must never be allowed to take down the real action
    that already succeeded earlier in the same request."""
    cur = conn.cursor()
    try:
        ...
        print(f"[notify] non-critical step failed, rolled back to savepoint: {exc}")
...
def submit_proposal(proposal_id: int, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    _safely(conn, _notify_approvers)
...
def approve_proposal(proposal_id: int, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    if p.get("submitted_by") and p["submitted_by"] != user["id"]:
        _safely(conn, lambda: _notify(conn, p["submitted_by"], f"Your budget \"{p['title']}\" was approved ✅"))
...
def reject_proposal(proposal_id: int, data: RejectRequest, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    if p.get("submitted_by") and p["submitted_by"] != user["id"]:
        ...
        _safely(conn, lambda: _notify(conn, p["submitted_by"], f"Your budget \"{p['title']}\" was rejected{reason_suffix}"))
