...
def submit_proposal(proposal_id: int, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    execute(conn,
        "UPDATE budget_proposals SET status='submitted', submitted_at=%s WHERE id=%s",
        (now, proposal_id)
    )
    # Recalculate total
    ...
    execute(conn, "UPDATE budget_proposals SET total_amount=%s WHERE id=%s", (total, proposal_id))
