...
class AssignMemberRequest(BaseModel):
    event_id: int
    user_id: int
    role: str = "volunteer"  # 'dept_head' or 'volunteer'
...
def get_department_roster(dept_id: int, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    coworkers = [m for m in members if m["role"] == "volunteer"]
...
def assign_department_member(dept_id: int, data: AssignMemberRequest, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    if not can_manage_departments(role_ctx) and role_ctx["level"] != "dept_head":
        raise HTTPException(status_code=403, detail="Only an Event Admin or Dept Head can assign team members")
    ...
    # If assigning as dept_head, update existing head in user_event_roles
    ...
    cur_existing = execute(conn, "SELECT id FROM user_event_roles WHERE user_id=%s AND event_id=%s", (data.user_id, data.event_id))
    existing = cur_existing.fetchone()

    if existing:
        execute(conn, """
            UPDATE user_event_roles SET role=%s, dept_id=%s, assigned_by=%s WHERE id=%s
        """, (data.role, dept_id, user["id"], existing["id"]))
    else:
        execute(conn, """
...
def remove_department_member(dept_id: int, user_id: int, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    role_ctx = get_event_role(conn, user, event_id)
    if not can_manage_departments(role_ctx) and role_ctx["level"] != "dept_head":
        raise HTTPException(status_code=403, detail="Only an Event Admin or Dept Head can remove team members")

    execute(conn, "UPDATE user_event_roles SET dept_id=NULL WHERE user_id=%s AND event_id=%s AND dept_id=%s", (user_id, event_id, dept_id))
    return {"ok": True}
