...
from utils.activity import log_activity, ACTION_EVENT_IMPORTED
import io
import openpyxl
...
async def import_event(event_id: int, file: UploadFile = File(...), conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    for idx, row in enumerate(ee_rows, start=2):
        ...
        if amount in (None, ""):
            errors.append(f"Estimated Expenses row {idx}: missing amount")
            continue
    ...
    for idx, row in enumerate(ae_rows, start=2):
        ...
        if amount in (None, ""):
            errors.append(f"Actual Expenses row {idx}: missing amount")
            continue
        category = category or "Other"
    ...
    for idx, row in enumerate(v_rows, start=2):
        ...
        try:
            ...
            cv = 0
        cur = execute(conn,
            "INSERT INTO vendors (event_id,name,category,contact_name,contact_email,contract_value,status,notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
            (event_id, name, category or "Other", contact_name or "", contact_email or "", cv, status or "active", notes or ""))
        vendor_id = cur.fetchone()["id"]
        if cv:
            def _sync_vendor(cv=cv, name=name, vendor_id=vendor_id):
                execute(conn,
                    ...
                    (event_id, name, f"Vendor contract: {name}", cv, f"vendor:{vendor_id}", f"Auto-synced from vendor #{vendor_id}"))
            run_safely(conn, _sync_vendor)
    ...
    imported_bits = [f"{v} {k.replace('_', ' ')}" for k, v in summary.items() if v]
    log_activity(conn, event_id, user["id"], ACTION_EVENT_IMPORTED,
                 f"{user['name']} imported " + (", ".join(imported_bits) if imported_bits else "a spreadsheet with no new records"))
