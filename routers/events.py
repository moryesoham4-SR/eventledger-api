...
from utils.activity import log_activity, ACTION_EVENT_STATUS_CHANGED
...
def get_events(conn=Depends(get_db), user=Depends(get_current_user)):
    # Deliberately the SAME query for every account, including super admins:
    # events you created, or events you were explicitly assigned to via
    # Assign Role. No org_name matching — see utils/roles.py docstring for why.
    cur = execute(conn,
...
def update_event(event_id: int, data: EventUpdate, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    if "status" in fields:
        log_activity(conn, event_id, user["id"], ACTION_EVENT_STATUS_CHANGED,
                     f"{user['name']} changed event status to \"{fields['status']}\"")
    return updated
