...
def my_role(event_id: int, conn=Depends(get_db), user=Depends(get_current_user)):
    ...
    return {
        "level": role_ctx["level"],  # "admin" | "finance" | "dept_head" | "volunteer" | None
        "dept_id": role_ctx["dept_id"],
        "can_manage_departments": role_ctx["level"] == "admin",
        "can_approve_budget": role_ctx["level"] in ("admin", "finance"),
    }
